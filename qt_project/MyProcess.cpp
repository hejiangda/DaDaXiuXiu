...
bool Process::cvAdd4cMat_q(cv::Mat& dst, cv::Mat& scr, double scale)
{
    ...
    for(auto x:scr_channels)
        x.release();
    for(auto x:dstt_channels)
        x.release();
    return true;
}
// 取文件名字 不包括后缀
...
//    cout<<(QString::fromStdString(folder) + "/透明图/" + QString::fromStdString(fileName) + "-T.png");
    imwrite((QString::fromStdString(folder) + "/透明图/" + QString::fromStdString(fileName) + "-T.png").toLocal8Bit().toStdString(), resultT, compression_params);
    imwrite((QString::fromStdString(folder) + "/白底图/" + QString::fromStdString(fileName) + "-W.jpg").toLocal8Bit().toStdString(), resultW);
    QString pngName=(QString::fromStdString(folder) + "/透明图/" + QString::fromStdString(fileName) + "-T.png");
    QProcess::execute("magick "+pngName+" -colors 255 "+pngName );
    cout<<"23";
    return true;
}
